...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(length=9),
            nullable=True,
            comment="Eventuale codice identificativo dell"
            "acqua "
    ...
    op.create_index(
        "acque_si1", "acque", ["geom"], schema="ctmp", postgresql_using="gist"
    )
    op.create_index(
        "acque_si2",
        "acque",
...
def downgrade() -> None:
    op.drop_index(
        "acque_i1", table_name="acque", schema="ctmp", postgresql_using="btree"
    )
    op.drop_index(
        "acque_si1", table_name="acque", schema="ctmp", postgresql_using="gist"
    )
    op.drop_index(
        "acque_si2", table_name="acque", schema="ctmp", postgresql_using="gist"
    )
    op.drop_index(
        "acque_si3", table_name="acque", schema="ctmp", postgresql_using="gist"
    )
