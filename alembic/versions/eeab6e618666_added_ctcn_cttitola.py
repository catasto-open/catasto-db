...
def upgrade() -> None:
    op.create_table('cttitola',
                    ...
                    sa.PrimaryKeyConstraint('codice', 'sezione', 'identifica', name='cttitola_pkey'),
                    schema='ctcn'
                    )
    op.create_index('cttitola_idx1', 'cttitola', ["codice", "sezione", "soggetto", "tipo_sog"],
                    schema='ctcn', postgresql_using='btree')
    op.create_index('cttitola_idx2', 'cttitola', ["codice", "sezione", "immobile", "tipo_imm"],
                    schema='ctcn', postgresql_using='btree')
    op.create_index('cttitola_soggetto_idx', 'cttitola', ["soggetto", "tipo_sog"],
                    schema='ctcn', postgresql_using='btree')


def downgrade() -> None:
    op.drop_index('cttitola_idx1', table_name='cttitola', schema='ctcn', postgresql_using='btree')
    op.drop_index('cttitola_idx2', table_name='cttitola', schema='ctcn', postgresql_using='btree')
    op.drop_index('cttitola_soggetto_idx', table_name='cttitola', schema='ctcn', postgresql_using='btree')
