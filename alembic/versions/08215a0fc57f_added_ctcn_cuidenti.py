...
def upgrade() -> None:
    ...
    op.create_index('cuidenti_idx1', 'cuidenti', ['codice', 'sezione', 'immobile', 'tipo_imm', 'progressiv'],
                    schema='ctcn', postgresql_using='btree')
    op.create_index('cuidenti_idx2', 'cuidenti', ['codice', 'sezione', 'foglio', 'numero'],
                    schema='ctcn', postgresql_using='btree')


def downgrade() -> None:
    op.drop_index('cuidenti_idx1', table_name='cuidenti', schema='ctcn', postgresql_using='btree')
    op.drop_index('cuidenti_idx2', table_name='cuidenti', schema='ctcn', postgresql_using='btree')
